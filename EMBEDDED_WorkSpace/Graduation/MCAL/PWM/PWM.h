...
#define TCNT0		(*(volatile uint8*)0x52)	/* register hold the timer Count and it is incremented on every timer "tick". */
...
void PWM_Start(uint8 mode,uint8 clk);		// This function Generate PWM Signal

/* mode is it Fast PWM or Phase Correct PWM */
/* clk is a Clock source which it start from 0 to 1024 prescaler */

void PWM_SetDuty(uint8 duty);			// This function Generate PWM signal with " duty " DutyCycle
